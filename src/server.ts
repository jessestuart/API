...
app.get('/historical/:country', async function(req, res) {
  let data = JSON.parse(await redis.get(keys.historical))
  const countryData = await scraper.historical.getHistoricalCountryData(
    data,
  ...
  let country = countries.find(e => {
    ...
    if (req.query.strict) {
      return req.query.strict.toLowerCase() == 'true'
        ? e.country.toLowerCase() === standardizedCountryName
        : e.country.toLowerCase().includes(standardizedCountryName)
    } else {
